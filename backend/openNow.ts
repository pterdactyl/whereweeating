...
/**
 * True if the restaurant is open at `date` in America/Toronto.
 * False when weeklyHours is present but has no valid interval covering now (incl. closed days).
 * True when weeklyHours is missing or not an object (caller should only pass when structured hours exist).
 */
...
/** When "prefer open now" is on: keep rows with no structured hours; drop known-closed. */
